...
#define ARGPARSE_VERSION 3

#ifdef _MSC_VER
#  define PACKED_STRUCT(name) \
    __pragma(pack(push, 1)) struct name __pragma(pack(pop))
#elif defined(__GNUC__)
//#  define PACKED_STRUCT(name) _Pragma("pack(1)") struct name _Pragma("pack()")
...
#define CONSTRUCTOR(T) T(int argc, char *argv[]) : argparse3::Args(argc, argv) {validate(_help);} bool _help = flag("help", "print help");
    ...
    template <typename T> struct ConvertType : public ConvertBase {
        ...
        ConvertType() = default;
        explicit ConvertType(T* &value) : data(value) {}
        explicit ConvertType(const T &value) : data(new T) {*data = value;}
        inline void convert(const std::string &v) override {
            *data = get<T>(v);
        }
        inline void set_default(const std::unique_ptr<ConvertBase> &default_value) override {
    ...
    struct Entry {
        ...
        Entry(ARG_TYPE type, const std::string& key, std::string help, void *class_ptr, std::size_t *global_offset, std::optional<std::string> implicit_value=std::nullopt) :
                type(type),
                keys(split(key)),
                help(std::move(help)),
                class_ptr(class_ptr),
        ...
        template <typename T> inline operator T() {
            T *p = (T*)((char *)class_ptr + *global_offset);
//            cout << "offset for " << _get_keys() << " : " << *global_offset << " ( size: " << sizeof(T) << ")" << endl;
            ...
            datap = std::make_unique<ConvertType<T>>(p);
            *global_offset += sizeof(T);
            return T{};
        };
    ...
    public:
        size_t global_offset = sizeof(Args);
        ...
        Entry &arg(const std::string &help) {
            std::shared_ptr<Entry> entry = std::make_shared<Entry>(Entry::ARG, "arg_" + std::to_string(_arg_idx++), help, this, &global_offset);
        ...
        Entry &kwarg(const std::string &key, const std::string &help, const std::optional<std::string>& implicit_value=std::nullopt) {
            std::shared_ptr<Entry> entry = std::make_shared<Entry>(Entry::KWARG, key, help, this, &global_offset, implicit_value);
            all_entries.emplace_back(entry);
         ...
         */
        void validate(const bool &_help) {
            parse(_help);
