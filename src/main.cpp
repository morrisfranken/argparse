...
struct MyArgs : public argparse::Args {
    ...
    std::optional<float> alpha  = kwarg("a,alpha", "An optional float parameter").set_default(std::optional<float>(std::nullopt));
    float beta                  = kwarg("b,beta", "An optional float parameter").set_default(0.6f);
    float gamma                 = kwarg("g,gamma", "An optional float parameter");
